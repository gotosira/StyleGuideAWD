...
$j(document).ready(function() {
    ...
    var lowest_price = function() {
        var i = 0, currency, result, arr = [], temp_actual_min = 0;

        $j('.tour_price').each(function() {

            var temp_value = $j(this).find('.tour_price_value').text();
            if(temp_value == 'N/A'){
                // skip to next array if N/A
                temp_value.replace(/,/g, '');
                temp_value = parseInt(temp_value);
                arr.push(temp_value);

                if(temp_actual_min == 0){
                    temp_actual_min = 0;
                }else{
                    // do nothing
                }
                // go to next value
                //alert('temp_actual_min is:' + temp_actual_min + 'arr' + '[' + i + '] is' + arr);
                i++;
            }else{
                temp_value.replace(/,/g, '');
                arr.push(temp_value);
                ...
                if ( temp_actual_min.replace(/,/g, '') > temp_value) {
                    temp_actual_min = $j(this).find('.tour_price_value').text();
                }
...
function portalOpenPageSearch() {
    ...
    if (typeof(aidSpecial) != 'undefined') url = url + '&aid=' + aidSpecial;
    if (typeof(bidSpecial) != 'undefined') url = url + '&bid=' + bidSpecial;
    window.open(url);
}
...
$j(document).ready(function() {
   ...
   /* if (ww <= 640) {
    ...
    // wrap link to banner ad
     if($j('.display_blocked_recommend, .display_block').length) {
        $j('.display_blocked_recommend, .display_block').wrap('<a href="http://www.asiawebdirect.com"></a>');
     }
